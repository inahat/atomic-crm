...
export const ContractStatus = ({ source }: { source: string }) => {
    ...
    return (
        <span className={`px-2 py-1 rounded-full text-xs font-semibold uppercase tracking-wide 
            ...
            ${status === 'Proposal-Sent' ? 'bg-indigo-100 text-indigo-800' : ''}
            ...
            ${!['Approved', 'Rejected', 'Open-Unbilled', 'OPEN-UNBILLED', 'Open-Billed', 'OPEN-BILLED', 'Proposal', 'Proposal-Sent', 'VIP'].includes(status) ? 'bg-gray-100 text-gray-800' : ''}
        `}>
            {status}
        </span>
    );

...
if (!storedUser || storedUser.toLowerCase().includes("test")) {
  localStorage.setItem(
    USER_STORAGE_KEY,
    JSON.stringify({ ...DEFAULT_USER, administrator: false })
  );
}
...
async function getUser(email: string) {
  ...
  if (!sales.data.length) {
    return { ...DEFAULT_USER, email, administrator: false };
  }
...
export const authProvider: AuthProvider = {
  ...
  getPermissions: () => {
    const userItem = localStorage.getItem(USER_STORAGE_KEY);
    const localUser = userItem ? (JSON.parse(userItem) as Sale) : null;
    if (!localUser) return Promise.resolve("user");
  ...
  getIdentity: () => {
    ...
    return Promise.resolve({
      id: user?.id ?? 0,
      fullName: user ? `${user.first_name} ${user.last_name}` : "Testy Test",
